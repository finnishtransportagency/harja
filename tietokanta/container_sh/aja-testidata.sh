...
psql -h "localhost" -p "${POSTGRESQL_PORTTI:-5432}" -U harja harja -c "CREATE DATABASE harjatest_template WITH TEMPLATE harja OWNER harjatest;";
echo "Luodaan harjatest kanta harjatest_template kannan pohjalta";
psql -h "localhost" -p "${POSTGRESQL_PORTTI:-5432}" -U harja harja -c "CREATE DATABASE harjatest WITH TEMPLATE harjatest_template OWNER harjatest;";