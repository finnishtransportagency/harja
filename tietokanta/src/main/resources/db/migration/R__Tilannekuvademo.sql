CREATE OR REPLACE FUNCTION ajoneuvo_suljetulle_tieosuudelle_test(suljettu_osuus_id INTEGER, tyokone INTEGER, organisaatio_ INTEGER, ajoneuvotyyppi VARCHAR, urakkaid INTEGER, tehtavat suoritettavatehtava[]) RETURNS VOID AS $$
BEGIN
  ...
  INSERT INTO tyokonehavainto (tyokoneid, jarjestelma, organisaatio, viestitunniste, lahetysaika, vastaanotettu, tyokonetyyppi, sijainti, urakkaid, edellinensijainti, suunta, tehtavat) 
       VALUES (tyokone, 'Jarjestelma', organisaatio_, 123, current_timestamp, current_timestamp, ajoneuvotyyppi, (SELECT ajoneuvo_sijainti FROM q), urakkaid, (SELECT ajoneuvo_sijainti FROM q), 0, tehtavat)
