...
BEGIN
  ...
  IF suunta_=1::bit THEN
     RETURN ST_Reverse(ST_Line_Substring(tmp, bpituus::FLOAT/kok_pituus::FLOAT, apituus::FLOAT/kok_pituus::FLOAT));
  ELSE
...
DECLARE
  tmp RECORD;
BEGIN
    SELECT k.osa, k.p FROM (SELECT osa, sum(pituus) OVER (PARTITION BY tie ORDER BY osa) AS p 
