    ...
    WHERE toimenpideinstanssi IN
          (select tpi.id
           from toimenpideinstanssi tpi
                    ...
                    join urakka u on tpi.urakka = u.id and u.tyyppi = 'teiden-hoito' and (u.loppupvm + interval '1 month' * 3) > pvm); -- Maksueriä voi lähettää Sampoon vielä 3 kk urakan päättymisen jälkeen.
