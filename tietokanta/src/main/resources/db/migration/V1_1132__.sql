...
BEGIN
    -- Looppaa kaikki urakat 
    FOR urakka_id IN
        SELECT id FROM urakka
    LOOP
        -- Loopataan kaikki tehtäväryhmät, mikä ei ole välttämätöntä mutta ei haittaakaan
        FOR tehtavaryhma_id IN
            SELECT DISTINCT tehtavaryhma FROM kulu_kohdistus
        LOOP
            ...
            IF toimenpideinstanssi_id IS NOT NULL THEN
                ...
                SET toimenpideinstanssi = toimenpideinstanssi_id,
                    muokattu = current_timestamp,
                    muokkaaja = (SELECT id FROM kayttaja WHERE kayttajanimi = 'Integraatio')
                -- Missä tehtäväryhmä on loopattu tehtäväryhmä
