CREATE OR REPLACE FUNCTION urakan_tarkka_perusluku(urakka_id INTEGER)
  RETURNS NUMERIC AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION laskutusyhteenveto_tarkalla_perusluvulla(
...
--   END IF;
  ...
  perusluku := urakan_tarkka_perusluku(ur);
...
-- tällä otettiin ne joilla perusluku oli eri
\copy (select u.id, u.nimi, (SELECT urakan_tarkka_perusluku(u.id)) as perusluku_vanha_tapa, (SELECT hoitourakan_indeksilaskennan_perusluku(u.id)) as perusluku_uusi_tapa,(SELECT SUM(lyv_v.kaikki_laskutetaan_ind_korotus) as ind_vanha_09_2016 FROM laskutusyhteenveto_tarkalla_perusluvulla('2015-10-01', '2016-09-30', '2016-09-01', '2016-09-30',u.id) lyv_v),(SELECT SUM(lyv.kaikki_laskutetaan_ind_korotus) as ind_uusi_09_2016 FROM laskutusyhteenveto('2015-10-01', '2016-09-30', '2016-09-01', '2016-09-30',u.id) lyv),(SELECT SUM(lyv_v.kaikki_laskutetaan_ind_korotus) as ind_vanha_10_2016 FROM laskutusyhteenveto_tarkalla_perusluvulla('2016-10-01', '2017-09-30', '2016-10-01', '2016-10-31',u.id) lyv_v),(SELECT SUM(lyv.kaikki_laskutetaan_ind_korotus) as ind_uusi_10_2016 FROM laskutusyhteenveto('2016-10-01', '2017-09-30', '2016-10-01', '2016-10-31',u.id) lyv),(SELECT SUM(lyv_v.kaikki_laskutetaan_ind_korotus) as ind_vanha_11_2016 FROM laskutusyhteenveto_tarkalla_perusluvulla('2016-10-01', '2017-09-30', '2016-11-01', '2016-11-30', u.id) lyv_v),(SELECT SUM(lyv.kaikki_laskutetaan_ind_korotus) as ind_uusi_11_2016 FROM laskutusyhteenveto('2016-10-01', '2017-09-30', '2016-11-01', '2016-11-30', u.id) lyv) FROM urakka u where u.id in (63,155,31,141,75,38,109,132,143,154,128,37,118,77,11,53,40,57,32,33,78,152,113,54,19,112,65,8,86,93,99,157,156,153,56,151,81,90,51,59,89,104,22,80,74,73,150,110,148,107,35,49,95) order by u.nimi) to '/tmp/indeksikorotukset-kaikki.csv' DELIMITER ',' CSV HEADER