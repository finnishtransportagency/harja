...
-- Muutetaan yllapitokohde taulun pkluokka TEXT -> Enum
--  1. Lisätään uusi tyyppi
CREATE TYPE korjausluokkatyyppi AS ENUM ('PK1', 'PK2', 'PK3', 'Ei tiedossa');

ALTER TABLE yllapitokohde RENAME COLUMN pkluokka TO _pkluokka;

ALTER TABLE yllapitokohde ADD pkluokka korjausluokkatyyppi;

UPDATE yllapitokohde SET pkluokka = _pkluokka :: TEXT :: korjausluokkatyyppi;

ALTER TABLE yllapitokohde DROP COLUMN _pkluokka;

-- Päivitetään ylläpitokohteelle laskettavan pkluokan tyyppi enumiksi
...
BEGIN
    ...
    -- ***************** Muuttunut kohta *****************
    UPDATE yllapitokohde SET pkluokka = ypkluokka::korjausluokkatyyppi WHERE id = yllapitokohde_id;
...
ALTER TABLE paikkauskohde
    ADD COLUMN pkluokka korjausluokkatyyppi;
