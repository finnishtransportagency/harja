-- Päivitä materiaalilistaus-view, alkuperäinen määrä on luontiajan mukainen ensimmäinen kirjaus
-- Aiemmin käytettiin kirjauksen pvm:ää, mutta jos samalle päivälle on useampi kirjaus, ei voida määrittää, mikä on alkuperäinen määrä
CREATE OR REPLACE VIEW vv_materiaalilistaus AS
  SELECT DISTINCT ON (m1."urakka-id", m1.nimi)
    ...
    (SELECT maara
     ...
     ORDER BY luotu ASC -- Luontiajan mukainen ensimmäinen kirjaus
