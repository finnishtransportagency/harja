...
-- Luodaan palautettava rivityyppi

-- huom: Maskuerälaskennassa käytetään jo näitä sarakkeita, ei poisteta ainakaan niitä!
...
--(laskutusyhteenveto_teiden_hoito).tpi

DROP FUNCTION IF EXISTS laskutusyhteenveto_teiden_hoito(
    hk_alkupvm date, hk_loppupvm date, aikavali_alkupvm date,
    aikavali_loppupvm date,
    ur integer);

DROP TYPE IF EXISTS laskutusyhteenveto_mhu_rivi;

CREATE TYPE laskutusyhteenveto_mhu_rivi
    AS (nimi VARCHAR, tuotekoodi VARCHAR, tpi INTEGER, perusluku NUMERIC,
    kaikki_laskutettu NUMERIC, kaikki_laskutetaan NUMERIC,
    kit_laskutettu  NUMERIC, kit_laskutetaan NUMERIC,
    kat_laskutettu  NUMERIC, kat_laskutetaan NUMERIC,
    kht_laskutettu  NUMERIC, kht_laskutetaan NUMERIC,
    mt_laskutettu NUMERIC, mt_laskutetaan NUMERIC,
    aht_laskutettu NUMERIC, aht_laskutetaan NUMERIC,

    sakot_laskutettu NUMERIC, sakot_laskutetaan NUMERIC,
    suolasakot_laskutettu NUMERIC, suolasakot_laskutetaan NUMERIC,
    bonukset_laskutettu NUMERIC,bonukset_laskutetaan NUMERIC,
...
CREATE OR REPLACE FUNCTION laskutusyhteenveto_teiden_hoito(
    hk_alkupvm date, hk_loppupvm date, aikavali_alkupvm date,
    aikavali_loppupvm date,
    ur integer) returns SETOF laskutusyhteenveto_mhu_rivi
    language plpgsql
as
$$
DECLARE
    ...
    rivi                                      laskutusyhteenveto_mhu_rivi;
BEGIN
    ...
    FOR t IN SELECT tpk2.nimi  AS nimi,
        ...
        LOOP
            ...
            kaikki_laskutettu := yht_laskutettu + sakot_laskutettu +
                                 COALESCE(suolasakot_laskutettu, 0.0) +
                                 aht_laskutettu +
                                 mt_laskutettu +
                                 bonukset_laskutettu + kht_laskutettu;

            kaikki_laskutetaan := yht_laskutetaan + sakot_laskutetaan +
    ...
    ', t.nimi;
            ...
            rivi := (t.nimi, t.tuotekoodi, t.tpi, perusluku,
                     ...
                     kit_laskutettu, kit_laskutetaan,
                     kat_laskutettu, kat_laskutetaan,
                     kht_laskutettu, kht_laskutetaan,
                     mt_laskutettu, mt_laskutetaan,
                     aht_laskutettu, aht_laskutetaan,
