-- kuvaus: tierekisteriosoitteelle viiva, korjattu versio
...
BEGIN
   IF aosa_ = losa_ THEN
	SELECT ST_LineMerge(ST_Union(ST_Line_Substring(geom, LEAST(1.0, aet_/ST_Length(geom)), LEAST(1.0,let_/ST_Length(geom))) ORDER BY osa,path))
