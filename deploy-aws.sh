...
aws ec2 run-instances --image-id ami-2d79a942 --key-name harja_upcloud_rsa --instance-type t2.medium --profile harja-dev > .deploy3
...
while [ -z "$HOST" ]; do
    HOST=`aws ec2 describe-instances --instance-ids --profile harja-dev $INSTANCE | jq -r ".Reservations[0].Instances[0].PublicDnsName"`;
    sleep 1;
done;
