...
mvn -f pom.vanha.xml clean compile flyway:migrate -Dflyway.url=jdbc:postgresql://localhost/harjatest_template -Dflyway.user=harjatest -Dflyway.password=$(cat ~/.pgpass | sed 's/.*harjatest://')
