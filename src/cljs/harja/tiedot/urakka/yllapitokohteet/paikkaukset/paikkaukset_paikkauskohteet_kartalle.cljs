...
(def paikkauskohteet-kartalla
  (let [;; Testataan mahdollisuutta näyttää vain valittu kohde kartalle
        ...
        kohteet (keep (fn [kohde]
                        (when
                          (and (or (nil? valitut-kohteet)
                                   (contains? valitut-kohteet (:testinro kohde)))
                            (:sijainti kohde))
                          kohde))
                      (:paikkauskohteet @karttataso-paikkauskohteet))
        _ (js/console.log "valitut-kohteet:" (pr-str valitut-kohteet)
                         "valitut-kohteet-atom:" (pr-str @valitut-kohteet-atom) )
        alueet (with-meta (mapv (fn [kohde]
                                  (when (:sijainti kohde)
                                    {:alue (:sijainti kohde)
                                     :tyyppi-kartalla :paikkaukset-paikkauskohteet
                                     :selite {:teksti "Paikkauskohde jee"
                                              :img (pinni-ikoni "sininen")}
                                     :infopaneelin-tiedot {:nro (:testinro kohde)
                                                           :nimi (:testinimi kohde)
                                                           :tila (:testitila kohde)
                                                           :menetelma (:testimenetelma kohde)
                                                           :aikataulu (:testiaikataulu kohde)}
                                     :ikonit [{:tyyppi :merkki
                                               :paikka [:loppu]
                                               :zindex 21
                                               :img (pinni-ikoni "sininen")}]}))
    ...
    (reaction
      (when (and (not (nil? alueet)) @karttataso-nakyvissa?)
        alueet))))