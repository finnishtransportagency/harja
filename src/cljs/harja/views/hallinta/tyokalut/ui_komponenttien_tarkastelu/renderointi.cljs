...
(defn renderoi-valilehdet-parametreista [{:keys [valilehtien-tilat]}
	...
	(let [valilehti-id (or data-cy "ui-komponenttien-tarkastelu-tabs")
			aktiivinen-valilehti (or (get @valilehtien-tilat valilehti-id)
								   (-> valilehdet first :avain))]
		(when-not (contains? @valilehtien-tilat valilehti-id)
			(swap! valilehtien-tilat assoc valilehti-id aktiivinen-valilehti))
		[:div {:class "ui-komponenttien-tarkastelu-valilehdet"
			   :data-cy valilehti-id}
		 (into [bootstrap/tabs {:active (r/cursor valilehtien-tilat [valilehti-id])
							  :style tyyli
							  :classes luokka}]
