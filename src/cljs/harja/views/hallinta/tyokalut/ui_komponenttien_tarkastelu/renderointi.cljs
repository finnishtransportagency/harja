		...
		[:section {:class "ui-komponenttien-tarkastelu-teemapaneeli"
		 ...
		 [:div {:class "ui-komponenttien-tarkastelu-tyylioverridet-otsikko"}
		  [:div
		   ...
		   [:p "Säädä ensin jaettuja perustokeneita. Näitä voi käyttää useissa komponenteissa ennen komponenttikohtaisia overrideja."]]
		...
		(let [aktiiviset-tyylioverridet (komponentin-aktiiviset-tyylioverridet konteksti komponentti)
			...
			[:section {:class "ui-komponenttien-tarkastelu-tyylioverridet"
			 ...
			 [:div {:class "ui-komponenttien-tarkastelu-tyylioverridet-otsikko"}
			  [:div
			   [:h4 "Tyylioverridet"]
			   [:p "Säädä arvoja, katso preview ja kopioi muuttunut override-map kehittäjälle."]]
			  [:div {:class "ui-komponenttien-tarkastelu-tyylioverridet-toiminnot"}
			   [:button {:type "button"
					 :class "nappi-toissijainen"
...
(defn renderoi-valilehdet-parametreista [{:keys [valilehtien-tilat]}
	...
	(r/with-let [paikallinen-valilehtien-tila (r/atom {})]
		(let [valilehtien-tila (or valilehtien-tilat paikallinen-valilehtien-tila)
			  valilehti-id (or data-cy "ui-komponenttien-tarkastelu-tabs")
			  aktiivinen-valilehti (or (get @valilehtien-tila valilehti-id)
							 (-> valilehdet first :avain))]
		  (when-not (contains? @valilehtien-tila valilehti-id)
			(swap! valilehtien-tila assoc valilehti-id aktiivinen-valilehti))
		  [:div {:class "ui-komponenttien-tarkastelu-valilehdet"
				 :data-cy valilehti-id}
		   (into [bootstrap/tabs {:active (r/cursor valilehtien-tila [valilehti-id])
								:style tyyli
								:classes luokka}]
