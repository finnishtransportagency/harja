...
;; Hieman likaisen oloista, mutta tätä käytetään sen takia, että SyoteTaytaAlas komponentitn on-blur eventissä
...
;; käyttäytymisistä.
(defonce ^:mutable rivit-alla_ nil)
...
(defsolu SyoteTaytaAlas
  [nappi-nakyvilla? nappia-painettu! toiminnot kayttaytymiset parametrit fmt fmt-aktiiviselle]
    ...
    (let [on-change (fn [arvo]
          ...
          on-key-down (fn [event]
                        #_(when (= "Enter" (.. event -key))
                          (.. event -target blur)))
          input-osa (assoc (solu/syote {:toiminnot (into {}
                                                         (map (fn [[k f]]
                                                                [k (fn [x]
        ...
        [:div.kustannus-syotto {#_#_:class (apply str (interpose " " luokat))
         ...
         [napit/yleinen-ensisijainen "Kopioi allaoleviin" (r/partial tayta-alas! this)
          {:luokka (str "kopioi-nappi button-primary-default "
           ...
           :tabindex 0}]
         [grid/piirra (assoc input-osa ::tama-komponentti this) #_(-> input-osa
...
(defn tyhja->syote-tayta-alas [tyhja {:keys [nappi-nakyvilla? nappia-painettu! toiminnot kayttaytymiset parametrit fmt fmt-aktiiviselle]}]
  (-> (->SyoteTaytaAlas (grid/hae-osa tyhja :id) nappi-nakyvilla? (or nappia-painettu! identity) toiminnot kayttaytymiset parametrit fmt fmt-aktiiviselle)
...
(defn suunnittellut-hankinnat-grid []
  (let [nyt (pvm/nyt)
        ...
        #_#_g (grid/aseta-root-fn g {:haku (fn [] (get-in @tila/suunnittelu-kustannussuunnitelma [:gridit :suunnittellut-hankinnat :grid]))
                                 :paivita! (fn [f]
                                             (swap! tila/suunnittelu-kustannussuunnitelma
                                                    (fn [tila]
                                                      (update-in tila [:gridit :suunnittellut-hankinnat :grid] f))))})]
                                      ...
                                      (grid/paivita-grid! (grid/get-in-grid kopio [::g-pohjat/data-yhteenveto])
                                                          ...
                                                          (fn [lapset]
                                                            [(gov/tyhja->laajenna (get lapset 0)
                                                                                  (fn [this auki?]
                                                                                    (t/laajenna-solua-klikattu this auki? "suunnittellut-hankinnat-taulukko"))
                                                                                  false
    ...
    (grid/paivita-grid! g :koko (fn [koko]
                                  (update koko :rivi (fn [rivi]
                                                       (assoc rivi :korkeudet {0 "40px"
                                                                               2 "40px"})))))
    #_(println "suunnittellut-hankinnat-grid GRIDIN MUOTO")
    #_(grid/pre-walk-grid! g
                         (fn [osa]
                           (println (or (grid/hae-osa osa :nimi)
                                        (grid/hae-osa osa :id)))))
    (grid/paivita-grid! g :parametrit (fn [parametrit] (assoc parametrit :id "suunnittellut-hankinnat-taulukko")))
                        ...
                        (fn [lapset]
                          [(gov/tyhja->laajenna (get lapset 0)
                           ...
                           (gov/tyhja->syote (get lapset 1)
                                             {:on-change (fn [arvo]
                                                           (when arvo
                                                             (t/paivita-solun-arvo {:paivitettava-asia :hoidonjohtopalkkio-yhteenveto
                                                                                    :arvo arvo
                                                                                    :solu solu/*this*})))
                                              :on-blur (fn [arvo]
                                                         (when arvo
                     ...
                     (grid/paivita-grid! rivi
                                         ...
                                         (fn [osat]
                                           [(gov/tyhja->teksti (get osat 0)
                                            ...
                                            (gov/tyhja->teksti (get osat 2) {:class #{"table-default"}})
                                            (gov/tyhja->teksti (get osat 3) {:class #{"table-default" "harmaa-teksti"}})])))
                   (grid/hae-grid (grid/get-in-grid g [::g-pohjat/data 0 ::g-pohjat/data-sisalto]) :lapset)))
    (grid/paivita-grid! (grid/get-in-grid g [::g-pohjat/yhteenveto])
                        :lapset
                        (fn [lapset]
                          [(gov/tyhja->teksti (get lapset 0) {:class #{"table-default" "table-default-sum"}})
...
(defn johto-ja-hallintokorvaus [e! jh-laskulla jh-yhteenveto toimistokulut menneet-toimistokulusuunnitelmat kuluva-hoitokausi indeksit suodatin johto-ja-hallintokorvaus-grid kantahaku-valmis?]
  [:<>
...
(defn hallinnolliset-toimenpiteet-sisalto [e!
  ...
  [:<>
   ...
   [johto-ja-hallintokorvaus e! johto-ja-hallintokorvaus-laskulla johto-ja-hallintokorvaus-yhteenveto toimistokulut (:toimistokulut menneet-vuodet) kuluva-hoitokausi indeksit suodatin johto-ja-hallintokorvaus-grid kantahaku-valmis?]
...
(defn kustannussuunnitelma*
  ...
  (komp/luo
    (komp/piirretty (fn [_]
                      ...
                      (go (let [g (johto-ja-hallintokorvaus-laskulla-grid)]
                            (t/paivita-raidat! (grid/osa-polusta g [::g-pohjat/data]))))
    ...
    (fn [e*! {:keys [kuluva-hoitokausi suodattimet] :as app}]
      ...
      (let [tavoite-ja-kattohinta-argumentit (select-keys app #{:tavoite-ja-kattohinta :kuluva-hoitokausi :indeksit})
        ...
        [:div#kustannussuunnitelma
         ...
         [:span.viiva-alas]
         [hallinnolliset-toimenpiteet-sisalto e! hallinnolliset-argumentit (get-in app [:gridit :hoidonjohtopalkkio :grid]) (get-in app [:gridit :johto-ja-hallintokorvaus :grid]) (:kantahaku-valmis? app)]]))))
