              ...
              (concat
                [^{:key (hash rivi)}
                 [:tr (merge
                        ...
                        {:class (str (if (odd? @row-index-atom) "pariton" "parillinen") " " (when avattava? "klikattava"))})
                  [:td {:style {:width "50%"}} (first rivi)]
                  [:td {:style {:width "15%"}} (str toteutunut-maara " " (:yksikko (first (second rivi))))]
                  [:td {:style {:width "15%"}} (if (= -1 suunniteltu-maara)
                                                 "---"
                                                 (str (or suunniteltu-maara 0) " " (:yksikko (first (second rivi)))))]
                  [:td {:style {:width "20%"}} (if (= -1 suunniteltu-maara)
                                                 "---"
                  ...
                  (mapcat
                    (fn [lapsi]
                      (let [_ (reset! row-index-atom (inc @row-index-atom))]
                        [^{:key (hash lapsi)}
                         [:tr {:class (str (if (odd? @row-index-atom) "pariton" "parillinen"))}
                          [:td {:style {:width "50%"}} [muokkaa-toteumaa-linkki e! (:toteuma_aika lapsi) (:toteuma_id lapsi)]]
                          [:td {:style {:width "15%"}} (str (:toteutunut lapsi) " " (:yksikko lapsi))]
                          [:td {:style {:width "15%"}} "---"]
                          [:td {:style {:width "20%"}} "---"]]]))
                    lapsi-rivit)
                  ...
                  (when (and
                    ...
                    [^{:key (str "toteuma-" (hash rivi))}
                     (do
                       ...
                       [:tr {:class (str (if (odd? @row-index-atom) "pariton" "parillinen"))}
                        [:td {:style {:width "50%"}} [muokkaa-toteumaa-linkki e! (:toteuma_aika (first (second rivi))) (:toteuma_id (first (second rivi)))]]
                        [:td {:style {:width "15%"}} (str (:toteutunut (first (second rivi))) " " (:yksikko (first (second rivi))))]
                        [:td {:style {:width "15%"}} "---"]
                        [:td {:style {:width "20%"}} "---"]])])))))
          r)]
    [:div.taulukko
     [:div.livi-grid
      [:table.grid
       [:thead
        [:tr
         [:th {:style {:width "50%"}} "Tehtävä"]
         [:th {:style {:width "15%"}} "Toteuma nyt"]
         [:th {:style {:width "15%"}} "Suunniteltu"]
         [:th {:style {:width "20%"}} "Totetuma-%"]]]
       [:tbody
        (doall
          (for [l ll]
            ^{:key (hash l)}
