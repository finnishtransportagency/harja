  ...
  (go
    (let ;; Hae geometriat
      [yllapitokohteen-geometriat (<! (k/post! :debug-hae-yllapitokohteen-geometriat {:id yllapitokohde-id}))
       #_(js/console.log "yllapitokohteen-geometriat:" (pr-str yllapitokohteen-geometriat))]
          ...
          (tasot/nayta-geometria! (keyword (str "yllapitokohteen-geometria-" idx))
            {:type :yllapitokohde-geometriat
             ...
             :alue (assoc (:geometria s)
                     ...
                     :stroke {:color "red"
                              :width 1})})))
      #_ (tiedot-kartta/keskita-kartta-alueeseen! (harja.geo/extent yllapitokohteen-geometriat)))))

(defn- hae-ja-nayta-pkluokkageometriat []
  (go
    (let ;; Hae geometriat
      [geometriat (<! (k/post! :debug-hae-pkluokkageometriat {}))
       #_ (js/console.log "pkluokkageometriat:" (pr-str geometriat))]

      ;; Piirrä geometriat kartalle
      (doseq [[idx s] (map-indexed vector geometriat)]
        (when (:geometria s)
          (tasot/nayta-geometria! (keyword (str "pk-geometria-" idx))
            {:type :pk-geometriat
             :nimi (str "PK geometriat-"idx)
             :alue (assoc (:geometria s)
                     ;:fill "black"
                     :radius 3
                     :stroke {:color "black"
                              :width 1})}))
        #_(tiedot-kartta/keskita-kartta-alueeseen! (harja.geo/extent-monelle yllapitokohteen-geometriat))
        ))))
      ...
      [:div.lomake
       ...
       [:div.row.lomakerivi
        [:div.form-group.col-xs-12.col-sm-6.col-md-5.col-lg-4
         [:button {:on-click #(do
                                (let []
                                  (hae-ja-nayta-pkluokkageometriat)))}
...
(defn tierekisteri []
  (komp/luo
   ...
   (fn []
     ...
     #_  [tr-haku]
      [:hr]
    #_  [:h3 "Koordinaattihaku"]
     #_  [koordinaatti-haku]
      [:hr]
     #_ [:h3 "Valitse kartalta"]
      #_  (if @valitse-kartalla?
        ...
        [:button {:on-click #(reset! valitse-kartalla? true)}
         "Valitse kartalla"])
      #_ (when-let [valittu @valittu-osoite]
        [:div (pr-str valittu)])
      #_ [tarkastus-payload]
      [:hr]
     #_  [:h3 "Reittipisteiden haku - toteumille, tyokonehavainnoille tai tarkastusajoille"]
      #_ [reittipisteiden-haku]
      #_ [reittitoteuma-payload]
      [:hr]
      [:h3 "Tietokannasta haetun geometrian piirto kartalle"]
      [geometrian-piirto]
      [:hr]
    #_  [:h3 "Piirrä piste kartalle"]
      #_  [pisteen-piirto]
    #_  [:h3 "Piirrä toinen kartalle"]
      #_ [pisteen-piirto2]
      [:hr]
     #_  [:h3 "Visualisoi urakan pohjavesien rajoitusalueet kartalle"]
      #_  [urakan-rajoitusalueet]
      [:hr]
     #_ [:h3 "Visualisoi suolatoteumat kartalle"]
      #_  [urakan-suolat]
     #_  [:hr]
     #_  [:h3 "Visualisoi urakan geometria kartalle (hoito, mhu ja valaistusurakoille)"]
      #_ [urakan-geometriat]
      [:h3 "Piirrä tieturvallisuusverkon geometria kartalle"]
       [tieturvallisuus-geometriat]
      [:h3 "Piirrä päällystyskohteen geometria kartalle"]
