...
(def debugger-state-atom (r/atom nil))

(defn tuck [app root-component]
  (r/with-let [_ (reset! debugger-state-atom (debug-state app root-component))
               _ (add-watch app ::outside-state-update (outside-state-update debugger-state-atom))
               _ (add-watch debugger-state-atom ::debug (debugger-state-update app))]

    [tuck/tuck debugger-state-atom debug-ui]
    (finally
      (remove-watch app ::outside-state-update)
      (remove-watch debugger-state-atom ::debug))))


(defn ^:export pretty-print-debugger-states []
  (when @debugger-state-atom
    (let [{:keys [states]} @debugger-state-atom]
      (doseq [[i state] (map-indexed vector states)]
        (println (with-out-str
                   (cljs.pprint/pprint
                     {:state-index i
                      :event (when (:event state)
                               (-> (:event state) type pr-str short-fq-name))
                      :state (:state state)})))))))

(defn ^:export pretty-print-current-state []
  (when @debugger-state-atom
    (let [{:keys [states current-state]} @debugger-state-atom
          last-state (dec (count states))
          state {:state-index last-state
                 :state (:state (nth states last-state))}]
