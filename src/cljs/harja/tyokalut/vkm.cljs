            ...
            (.setAttribute "src" (str (vkm-base-url) uri
                                      (map->parametrit
                                       (assoc parametrit
                                         :callback callback)))))
        ch (chan)
        tulos #(do (put! ch %) ;; FIXME: JSON->CLJ muunnos
                   (close! ch)
...
(defn tieosoite [parametrit]
  "Kutsuu tieosoite palvelua, parametrit voi sisältää avaimet:

  :tie       tien numero
  :osa       tien osa
  :etaisyys  etäisyys tien alusta
  :losa      loppuosa
  :let       loppuetäisyys
...
Palautettavassa datassa:
  ...
  (vkm-kutsu "tieosoite" parametrit))
