...
(defn- palauteohje-kayttooikeus [palaute-tyyppi]
  [:div
   [:div "Jos käyttäjältä puuttuu käyttäjätunnukset Harjaan, ole yhteydessä oman organisaatiosi pääkäyttäjään. Jos et tiedä kuka organisaatiosi pääkäyttäjä on, ole yhteydessä Väyläviraston Käyttövaltuushallintaan osoitteessa jl_kvhtuki@vayla.fi"]
   [modal/modal-linkki
...
(defn- palauteohje-tehtavalista [palaute-tyyppi]
  [:span "Harjan pääkäyttäjä vastaa Harjan tehtävälistasta. "
   [modal/modal-linkki
...
(defn- palautelomake []
  (let [valinta-atom (atom nil)]
    (fn []
      [:div
       [:div "Valitse, mitä palautteesi koskee:"]
       ...
       [:div "Palautteen voit lähettää sähköpostitse osoitteeseen "
...
(defn palaute-linkki []
  [:a {:href "#"
       ...
       :on-click #(do
                    (.preventDefault %)
                    (modal/nayta! {:otsikko "Palautteen lähettäminen"
                                   :luokka "palaute-dialog"}
                      [palautelomake]))}
   [:span.nav-link-icon [:i.icon.ti.ti-message-forward]]
   "Palautetta!"])
