...
;; Open iconi
;;FIXME: Vaikuttaa siltä, että tässä osiossa neljä alinta ei ole ollenkaan käytössä. Kaksi ylintä ei myöskään koskaan renderöidy näytölle, niiden sijaan renderöityy livicon-chevron-up ja livicon-chevron-down. Voisi poistaa koodista kokonaan.
(defn oi-caret-top []
  [:img {:src "open-iconic/svg/caret-top.svg"}])
(defn oi-caret-bottom []
  [:img {:src "open-iconic/svg/caret-bottom.svg"}])
(defn oi-caret-left []
  [:img {:src "open-iconic/svg/caret-left.svg"}])
(defn oi-caret-right []
  [:img {:src "open-iconic/svg/caret-right.svg"}])
(defn oi-x []
  [:img {:src "open-iconic/svg/x.svg"}])
(defn oi-trash []
  [:img {:src "open-iconic/svg/trash.svg"}])
...
(defn livicon-ban []
  [:span.livicon-ban])
;; Nuolia chevron-up, chevron-down, chevron-right, chevron-left on käytetty ilmaisemaan ylöspäin siirtyimistä ja avattua tai suljettua haitarielementtiä (esim. kustannussuunnitelmassa on molempia käytäntöjä)
...
(defn livicon-back-circle []
  [:img {:src "images/harja-icons/action/back-circle.svg" :alt "peruuta" :aria-label "peruuta" :style {:padding-bottom "1px"}}])

(defn klikattava-roskis [toiminto]
  [:span.klikattava
   {:on-click toiminto :alt "poista" :aria-label "poista"}
   (livicon-trash)])
...
;; Sää ikonit 
;; Ruudunlukija ei päädy työmaapäiväkirjan säätietotaulukkoon, joten aria-label on ainakin siinä mielessä turha.
(defn weather-clear-day [] [:img.saa-ikonit.clear-day {:src "images/harja-icons/weather/clear-day.svg" :alt "selkeää päivällä" :aria-label "selkeää päivällä"}])
(defn weather-clear-night [] [:img.saa-ikonit.clear-night {:src "images/harja-icons/weather/clear-night.svg" :alt "selkeää yöllä" :aria-label "selkeää yöllä"}])
(defn weather-cloudy-1-day [] [:img.saa-ikonit.cloudy-1-day {:src "images/harja-icons/weather/cloudy-1-day.svg" :alt "enimmäkseen selkeää päivällä" :aria-label "enimmäkseen selkeää päivällä"}])
(defn weather-cloudy-1-night [] [:img.saa-ikonit.cloudy-1-night {:src "images/harja-icons/weather/cloudy-1-night.svg" :alt "enimmäkseen selkeää yöllä" :aria-label "enimmäkseen selkeää yöllä"}])
(defn weather-cloudy-2-day [] [:img.saa-ikonit.cloudy-2-day {:src "images/harja-icons/weather/cloudy-2-day.svg" :alt "puolipilvistä päivällä" :aria-label "puolipilvistä päivällä"}])
(defn weather-cloudy-2-night [] [:img.saa-ikonit.cloudy-2-night {:src "images/harja-icons/weather/cloudy-2-night.svg" :alt "puolipilvistä yöllä" :aria-label "puolipilvistä yöllä"}])
(defn weather-cloudy-3-day [] [:img.saa-ikonit.cloudy-3-day {:src "images/harja-icons/weather/cloudy-3-day.svg" :alt "enimmäkseen pilvistä päivällä" :aria-label "enimmäkseen pilvistä päivällä"}])
(defn weather-cloudy-3-night [] [:img.saa-ikonit.cloudy-3-night {:src "images/harja-icons/weather/cloudy-3-night.svg" :alt "enimmäkseen pilvistä yöllä" :aria-label "enimmäkseen pilvistä yöllä"}])
(defn weather-cloudy [] [:img.saa-ikonit.cloudy {:src "images/harja-icons/weather/cloudy.svg" :alt "pilvistä" :aria-label "pilvistä"}])
(defn weather-dust [] [:img.saa-ikonit.dust {:src "images/harja-icons/weather/dust.svg" :alt "pölyä" :aria-label "pölyä"}])
(defn weather-fog-day [] [:img.saa-ikonit.fog-day {:src "images/harja-icons/weather/fog-day.svg" :alt "sumua päivällä" :aria-label "sumua päivällä"}])
(defn weather-fog-night [] [:img.saa-ikonit.fog-night {:src "images/harja-icons/weather/fog-night.svg" :alt "sumua yöllä" :aria-label "sumua yöllä"}])
(defn weather-fog [] [:img.saa-ikonit.fog {:src "images/harja-icons/weather/fog.svg" :alt "sumua" :aria-label "sumua"}])
(defn weather-frost-day [] [:img.saa-ikonit.frost-day {:src "images/harja-icons/weather/frost-day.svg" :alt "hallaa päivällä" :aria-label "hallaa päivällä"}])
(defn weather-frost-night [] [:img.saa-ikonit.frost-night {:src "images/harja-icons/weather/frost-night.svg" :alt "hallaa yöllä" :aria-label "hallaa yöllä"}])
(defn weather-frost [] [:img.saa-ikonit.frost {:src "images/harja-icons/weather/frost.svg" :alt "sumua tai jääsumua" :aria-label "paikoin sumua tai jääsumua"}])
(defn weather-hail [] [:img.saa-ikonit.hail {:src "images/harja-icons/weather/hail.svg" :alt "raekuuroja" :aria-label "raekuuroja"}])
(defn weather-haze-day [] [:img.saa-ikonit.haze-day {:src "images/harja-icons/weather/haze-day.svg" :alt "utua päivällä" :aria-label "utua päivällä"}])
(defn weather-haze-night [] [:img.saa-ikonit.haze-night {:src "images/harja-icons/weather/haze-night.svg" :alt "utua yöllä" :aria-label "utua yöllä"}])
(defn weather-haze [] [:img.saa-ikonit.haze {:src "images/harja-icons/weather/haze.svg" :alt "utua" :aria-label "utua"}])
(defn weather-hurricane [] [:img.saa-ikonit.hurricane {:src "images/harja-icons/weather/hurricane.svg" :alt "hirmumyrsky" :aria-label "hirmumyrsky"}])
(defn weather-isolated-thunderstorms-day [] [:img.saa-ikonit.isolated-thunderstorms-day {:src "images/harja-icons/weather/isolated-thunderstorms-day.svg" :alt "yksittäisiä ukkoskuuroja päivällä" :aria-label "yksittäisiä ukkoskuuroja päivällä"}])
(defn weather-isolated-thunderstorms-night [] [:img.saa-ikonit.isolated-thunderstorms-night {:src "images/harja-icons/weather/isolated-thunderstorms-night.svg" :alt "yksittäisiä ukkoskuuroja yöllä" :aria-label "yksittäisiä ukkoskuuroja yöllä"}])
(defn weather-isolated-thunderstorms [] [:img.saa-ikonit.isolated-thunderstorms {:src "images/harja-icons/weather/isolated-thunderstorms.svg" :alt "yksittäisiä ukkoskuuroja" :aria-label "yksittäisiä ukkoskuuroja"}])
(defn weather-rain-and-sleet-mix [] [:img.saa-ikonit.rain-and-sleet-mix {:src "images/harja-icons/weather/rain-and-sleet-mix.svg" :alt "räntäsadetta" :aria-label "räntäsadetta"}])
(defn weather-rain-and-snow-mix [] [:img.saa-ikonit.rain-and-snow-mix {:src "images/harja-icons/weather/rain-and-snow-mix.svg" :alt "räntäsadetta" :aria-label "räntäsadetta"}])
(defn weather-rainy-1-day [] [:img.saa-ikonit.rainy-1-day {:src "images/harja-icons/weather/rainy-1-day.svg" :alt "heikkoa vesisadetta päivällä" :aria-label "heikkoa vesisadetta päivällä"}])
(defn weather-rainy-1-night [] [:img.saa-ikonit.rainy-1-night {:src "images/harja-icons/weather/rainy-1-night.svg" :alt "heikkoa vesisadetta yöllä" :aria-label "heikkoa vesisadetta yöllä"}])
(defn weather-rainy-1 [] [:img.saa-ikonit.rainy-1 {:src "images/harja-icons/weather/rainy-1.svg" :alt "heikkoa vesisadetta" :aria-label "heikkoa vesisadetta"}])
(defn weather-rainy-2-day [] [:img.saa-ikonit.rainy-2-day {:src "images/harja-icons/weather/rainy-2-day.svg" :alt "kohtalaista vesisadetta päivällä" :aria-label "kohtalaista vesisadetta päivällä"}])
(defn weather-rainy-2-night [] [:img.saa-ikonit.rainy-2-night {:src "images/harja-icons/weather/rainy-2-night.svg" :alt "kohtalaista vesisadetta yöllä" :aria-label "kohtalaista vesisadetta yöllä"}])
(defn weather-rainy-2 [] [:img.saa-ikonit.rainy-2 {:src "images/harja-icons/weather/rainy-2.svg" :alt "kohtalaista vesisadetta" :aria-label "kohtalaista vesisadetta"}])
(defn weather-rainy-3-day [] [:img.saa-ikonit.rainy-3-day {:src "images/harja-icons/weather/rainy-3-day.svg" :alt "voimakasta vesisadetta päivällä" :aria-label "voimakasta vesisadetta päivällä"}])
(defn weather-rainy-3-night [] [:img.saa-ikonit.rainy-3-night {:src "images/harja-icons/weather/rainy-3-night.svg" :alt "voimakasta vesisadetta yöllä" :aria-label "voimakasta vesisadetta yöllä"}])
(defn weather-rainy-3 [] [:img.saa-ikonit.rainy-3 {:src "images/harja-icons/weather/rainy-3.svg" :alt "voimakasta vesisadetta" :aria-label "voimakasta vesisadetta"}])
(defn weather-rainy-4 [] [:img.saa-ikonit.rainy-4 {:src "images/harja-icons/weather/rainy-4.svg" :alt "erittäin voimakasta vesisadetta" :aria-label "erittäin voimakasta vesisadetta"}])
(defn weather-scattered-thunderstorms-day [] [:img.saa-ikonit.scattered-thunderstorms-day {:src "images/harja-icons/weather/scattered-thunderstorms-day.svg" :alt "ukkoskuuroja päivällä" :aria-label "ukkoskuuroja päivällä"}])
(defn weather-scattered-thunderstorms-night [] [:img.saa-ikonit.scattered-thunderstorms-night {:src "images/harja-icons/weather/scattered-thunderstorms-night.svg" :alt "ukkoskuuroja yöllä" :aria-label "ukkoskuuroja yöllä"}])
(defn weather-scattered-thunderstorms [] [:img.saa-ikonit.scattered-thunderstorms {:src "images/harja-icons/weather/scattered-thunderstorms.svg" :alt "ukkoskuuroja" :aria-label "ukkoskuuroja"}])
(defn weather-severe-thunderstorm [] [:img.saa-ikonit.severe-thunderstorm {:src "images/harja-icons/weather/severe-thunderstorm.svg" :alt "ukkosmyrsky" :aria-label "ukkosmyrsky"}])
(defn weather-snow-and-sleet-mix [] [:img.saa-ikonit.snow-and-sleet-mix {:src "images/harja-icons/weather/snow-and-sleet-mix.svg" :alt "räntäsadetta" :aria-label "räntäsadetta"}])
(defn weather-snowy-1-day [] [:img.saa-ikonit.snowy-1-day {:src "images/harja-icons/weather/snowy-1-day.svg" :alt "heikkoa lumisadetta päivällä" :aria-label "heikkoa lumisadetta päivällä"}])
(defn weather-snowy-1-night [] [:img.saa-ikonit.snowy-1-night {:src "images/harja-icons/weather/snowy-1-night.svg" :alt "heikkoa lumisadetta yöllä" :aria-label "heikkoa lumisadetta yöllä"}])
(defn weather-snowy-1 [] [:img.saa-ikonit.snowy-1 {:src "images/harja-icons/weather/snowy-1.svg" :alt "heikkoa lumisadetta" :aria-label "heikkoa lumisadetta"}])
(defn weather-snowy-2-day [] [:img.saa-ikonit.snowy-2-day {:src "images/harja-icons/weather/snowy-2-day.svg" :alt "kohtalaista lumisadetta päivällä" :aria-label "kohtalaista lumisadetta päivällä"}])
(defn weather-snowy-2-night [] [:img.saa-ikonit.snowy-2-night {:src "images/harja-icons/weather/snowy-2-night.svg" :alt "kohtalaista lumisadetta yöllä" :aria-label "kohtalaista lumisadetta yöllä"}])
(defn weather-snowy-2 [] [:img.saa-ikonit.snowy-2 {:src "images/harja-icons/weather/snowy-2.svg" :alt "kohtalaista lumisadetta" :aria-label "kohtalaista lumisadetta"}])
(defn weather-snowy-3-day [] [:img.saa-ikonit.snowy-3-day {:src "images/harja-icons/weather/snowy-3-day.svg" :alt "runsasta lumisadetta päivällä" :aria-label "runsasta lumisadetta päivällä"}])
(defn weather-snowy-3-night [] [:img.saa-ikonit.snowy-3-night {:src "images/harja-icons/weather/snowy-3-night.svg" :alt "runsasta lumisadetta yöllä" :aria-label "runsasta lumisadetta yöllä"}])
(defn weather-snowy-3 [] [:img.saa-ikonit.snowy-3 {:src "images/harja-icons/weather/snowy-3.svg" :alt "runsasta lumisadetta" :aria-label "runsasta lumisadetta"}])
(defn weather-thunderstorms [] [:img.saa-ikonit.thunderstorms {:src "images/harja-icons/weather/thunderstorms.svg" :alt "ukkosta" :aria-label "ukkosta"}])
(defn weather-tornado [] [:img.saa-ikonit.tornado {:src "images/harja-icons/weather/tornado.svg" :alt "pyörremyrsky" :aria-label "pyörremyrsky"}])
(defn weather-tropical-storm [] [:img.saa-ikonit.tropical-storm {:src "images/harja-icons/weather/tropical-storm.svg" :alt "trooppinen myrsky" :aria-label "trooppinen myrsky"}])
(defn weather-wind [] [:img.saa-ikonit.wind {:src "images/harja-icons/weather/wind.svg" :alt "tuulta" :aria-label "tuulta"}])
...
(defn harja-icon-action-add-attachment [] [:span.harja-icon-action-add-attachment {:title "lisää liite" :aria-label "lisää liite"}])
(defn harja-icon-action-add-picture [] [:span.harja-icon-action-add-picture {:title "lisää kuva" :aria-label "lisää kuva"}])
(defn harja-icon-action-add [] [:span.harja-icon-action-add {:title "lisää" :aria-label "lisää"}])
...
(defn harja-icon-action-clear [] [:span.harja-icon-action-clear {:title "tyhjennä" :aria-label "tyhjennä"}])
...
(defn harja-icon-action-log-in [] [:span.harja-icon-action-log-in {:title "kirjaudu sisään" :aria-label "kirjaudu sisään"}])
(defn harja-icon-action-log-out [] [:span.harja-icon-action-log-out {:title "kirjaudu ulos" :aria-label "kirjaudu ulos"}])
(defn harja-icon-action-message [] [:span.harja-icon-action-message])
(defn harja-icon-action-message-filled [] [:span.harja-icon-action-message-filled {:title "näytä kommentti" :aria-label "näytä kommentti"}])
...
(defn harja-icon-action-save [] [:span.harja-icon-action-save {:title "tallenna" :aria-label "tallenna"}])
(defn harja-icon-action-search [] [:span.harja-icon-action-search {:title "hae" :aria-label "hae"}])
(defn harja-icon-action-send-email [] [:span.harja-icon-action-send-email {:title "lähetä sähköposti" :aria-label "lähetä sähköposti"}])
(defn harja-icon-action-set-date [] [:span.harja-icon-action-set-date {:title "aseta päivämäärä" :aria-label "aseta päivämäärä"}])
(defn harja-icon-action-set-favorite [] [:span.harja-icon-action-set-favorite])
(defn harja-icon-action-set-time [] [:span.harja-icon-action-set-time {:title "aseta aika" :aria-label "aseta aika"}])
(defn harja-icon-action-show-list [] [:span.harja-icon-action-show-list])
(defn harja-icon-action-show-map [] [:span.harja-icon-action-show-map {:title "näytä kartta" :aria-label "näytä kartta"}])
(defn harja-icon-action-show [] [:span.harja-icon-action-show {:title "näytä" :aria-label "näytä"}])
(defn harja-icon-action-sort-ascending [] [:span.harja-icon-action-sort-ascending {:title "järjestä nousevasti" :aria-label "järjestä nousevasti"}])
(defn harja-icon-action-sort-descending [] [:span.harja-icon-action-sort-descending {:title "järjestä laskevasti" :aria-label "järjestä laskevasti"}])
(defn harja-icon-action-sound-on [] [:span.harja-icon-action-sound-on])
(defn harja-icon-action-subtract [] [:span.harja-icon-action-subtract {:title "vähennä" :aria-label "vähennä"}])
(defn harja-icon-action-undo [] [:span.harja-icon-action-undo {:title "peruuta" :aria-label "peruuta"}])
...
(defn harja-icon-misc-clock [] [:span.harja-icon-misc-clock {:title "aika" :aria-label "aika"}])
...
(defn harja-icon-navigation-close [] [:span.harja-icon-navigation-close {:title "sulje" :aria-label "sulje"}])
...
(defn harja-icon-navigation-external-link [] [:span.harja-icon-navigation-external-link {:title "ulkoinen linkki" :aria-label "ulkoinen linkki"}])
...
(defn harja-icon-navigation-home [] [:span.harja-icon-navigation-home {:title "etusivulle" :aria-label "etusivulle"}])
...
(defn harja-icon-navigation-more [] [:span.harja-icon-navigation-more {:title "lisää" :aria-label "lisää"}])
(defn harja-icon-navigation-next-page [] [:span.harja-icon-navigation-next-page {:title "seuraava" :aria-label "seuraava"}])
(defn harja-icon-navigation-notifications [] [:span.harja-icon-navigation-notifications])
(defn harja-icon-navigation-previous-page [] [:span.harja-icon-navigation-previous-page {:title "edellinen" :aria-label "edellinen"}])
...
(defn harja-icon-status-alert [] [:span.harja-icon-status-alert {:title "huomio" :aria-label "huomio"}])
...
(defn harja-icon-status-error [] [:span.harja-icon-status-error {:title "virhe" :aria-label "virhe"}])
