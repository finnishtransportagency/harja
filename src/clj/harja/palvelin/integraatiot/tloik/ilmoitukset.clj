...
(defn hae-urakka [db {:keys [urakkatyyppi sijainti]}]
  (let [urakka-id
             (condp contains? urakkatyyppi
               #{"paallystys" "paikkaus" "tiemerkinta"} (println "hae palvelusopimus")
               #{"valaistus"} (println "hae valaistusurakka")
               #{"hoito"} (println "hae hoitourakka"))
             ]

    (first (urakat/hae-urakka db urakka-id)))

  #_(when-let [urakka-id (first (urakkapalvelu/hae-urakka-idt-sijainnilla db urakkatyyppi sijainti))]
      (first (urakat/hae-urakka db urakka-id)))
  )
