...
(defn paivita-mhu-urakka-oid*                               ; pitää palauttaa closure, jossa db, ... jne.
  []
  #_(fn [konteksti]                                       ; Tämä palautetaan...
    (let [token-virhe-fn (fn [x] (log/error "Virhe Velho token haussa: " x))
          token-fn (fn [] (hae-velho-token token-url varuste-kayttajatunnus varuste-salasana konteksti token-virhe-fn))
          token (token-fn)]
      (when token
...
(defn paivita-mhu-urakka-oidt-velhosta
  ...
  (try+
    (integraatiotapahtuma/suorita-integraatio
      ...
      (fn [konteksti]
        #_(paivita-mhu-urakka-oid* konteksti db varuste-urakka-oid-url varuste-urakka-kohteet-url)))
    (catch [:type virheet/+ulkoinen-kasittelyvirhe-koodi+] {:keys [virheet]}
