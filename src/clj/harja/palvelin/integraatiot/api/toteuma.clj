...
(defn poista-toteuman-tehtavat-ulkoisella-idlla
  ...
  [db kayttaja-id ulkoinen-id urakka-id]
  (when-let [toteuma (first (q-toteumat/hae-toteuman-id-ulkoisella-idlla
                              db
                              {:ulkoinen_id ulkoinen-id
                               :urakka-id urakka-id}))]
    (let [toteuma-id (:id toteuma)]
  ...
  (jdbc/with-db-transaction [db db]
    (let [kayttaja-id (:id kirjaaja)
          ...
          _ (doseq [ulkoinen-id ulkoiset-idt]
              (poista-toteuman-tehtavat-ulkoisella-idlla db kayttaja-id ulkoinen-id urakka-id)
              (poista-toteuman-materiaalit-ulkoisella-idlla db kayttaja-id ulkoinen-id urakka-id))
