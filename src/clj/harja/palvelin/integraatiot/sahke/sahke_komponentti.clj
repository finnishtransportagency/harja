...
(defn laheta-urakka [lahettaja db urakka-id]
  ...
  (let [viesti-id (str (UUID/randomUUID))
        ...
        muodosta-xml #(urakkasanoma/muodosta urakka)]
    (try
      (jms-lahettaja muodosta-xml viesti-id)
      (log/debug (format "Urakan (id: %s) lähetys Sähkeeseen onnistui." id))
      ;; todo: merkitse urakalle lähetysaika
      (catch Exception e
        (log/error e (format "Urakan (id: %s) lähetys Sähkeeseen epäonnistui." id))
