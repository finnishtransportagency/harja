...
(defn- kulut-urakalle
  ...
  (let [kuukausi-valittu? (pvm/kyseessa-kk-vali? alkupvm loppupvm)
        ...
        otsikot [{:leveys 1 :otsikko "Tehtäväryhmä"}
                 {:leveys 1 :fmt :numero :otsikko (str "Hoitokauden alusta " (pvm/pvm alkupvm-hoitokausi) "-" (pvm/pvm loppupvm-hoitokausi))}
                 {:leveys 1 :fmt :numero :otsikko (cond
    ...
    [:raportti {:nimi (str "Kulut tehtäväryhmittäin" (when testiversio? " - TESTIVERSIO"))}
     ...
     [:taulukko
      {:otsikko                    "Urakkavuoden alusta"
       :viimeinen-rivi-yhteenveto? true}
      [{:leveys 1 :otsikko ""} {:leveys 1 :otsikko "" :fmt :numero}]
