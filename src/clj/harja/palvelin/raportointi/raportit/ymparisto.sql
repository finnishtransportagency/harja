...
SELECT
  -- haetaan käytetyt määrät per materiaali ja kk
  u.id                           AS urakka_id,
...
GROUP BY
  ...
  mk.id,
  mk.nimi,
  date_trunc('month', t.alkanut)
UNION
SELECT
  -- Haetaan reittipisteiden toteumat hoitoluokittain
  u.id                           AS urakka_id,
  u.nimi                         AS urakka_nimi,
...
UNION
SELECT
  -- Haetaan suunnitelmat materiaaleille
  u.id         AS urakka_id,
  u.nimi       AS urakka_nimi,
  NULL         AS luokka,
  mk.id        AS materiaali_id,
...
-- VANHA KYSELY,
--  jätetään tänne vielä tulevaisuutta varten,
--  jos löytyy eroavaisuuksia
SELECT *
  ...
  (SELECT date_trunc('month', kk + INTERVAL '1 month')
   ...
   WHERE kk + INTERVAL '1 month' < :loppupvm)),
      ...
      SELECT
        ...
        (SELECT SUM(rm.maara)
         ...
         WHERE rm.materiaalikoodi = mk.id
               ...
               AND (:urakka_annettu IS TRUE OR (:urakka_annettu IS FALSE AND
                                                (:urakkatyyppi :: urakkatyyppi IS NULL OR
                                                 u1.tyyppi = :urakkatyyppi :: urakkatyyppi)))
      ...
      SELECT
        ...
        NULL                                                    AS luokka,
        NULL                                                    AS kk,
        -- tyhjä kk pvm kertoo suunnitelman
        mks.id                                                  AS materiaali_id,
        mks.nimi                                                AS materiaali_nimi,
        mk.yksikko                                              AS materiaali_yksikko,
        (SELECT SUM(s.maara)
         FROM materiaalin_kaytto s
...
WHERE -- Poistetaan hoitoluokitellut rivit,
  --  joilla ei ole määrää
  NOT (luokka IS NOT NULL AND maara IS NULL);

-- name: hae-ymparistoraportti-urakoittain
-- Hakee ympäristöraportin,
--  mutta jokaiselle urakalle on omat rivinsä
SELECT *
FROM (WITH RECURSIVE kuukaudet (kk) AS (
  -- Haetaan kaikki kuukaudet alkupvm-loppupvm välillä
  VALUES (date_trunc('month', :alkupvm :: DATE))
   ...
   WHERE kk + INTERVAL '1 month' < :loppupvm)),
          urakat AS (
            ...
            WHERE (:alkupvm BETWEEN alkupvm AND loppupvm
                  ...
                  AND
                  (:urakkatyyppi :: urakkatyyppi IS NULL OR tyyppi = :urakkatyyppi :: urakkatyyppi)
        ),
          hoitoluokat AS (
          --      Is   I    Ib   TIb  II  III  K1   K2
          VALUES (1), (2), (3), (4), (5), (6), (7), (8)
