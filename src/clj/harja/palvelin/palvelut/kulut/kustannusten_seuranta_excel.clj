      ...
      (mapcat
        (fn [tehtava]
          (let [toteutunut-summa (or (:toteutunut_summa tehtava) 0)
                budjetoitu-summa (or (:budjetoitu_summa_indeksikorjattu tehtava) 0)
  ...
  (mapcat
    (fn [rivi]
      (let [toteutunut-summa (or (:toteutunut_summa rivi) 0)
            budjetoitu-summa (or (:budjetoitu_summa_indeksikorjattu rivi) 0)]
        [{:paaryhma nil
          :toimenpide nil
  ...
  (let [toimenpide-rivit
        (mapcat (fn [toimenpide]
                  (let [toimenpide-tot (or (:toimenpide-toteutunut-summa toimenpide) 0)
                        toimenpide-bud (or (:toimenpide-budjetoitu-summa-indeksikorjattu toimenpide) 123)
...
(defn- luo-excel-rivi-yhteensa [kustannusdata]
  (let [bud (get-in kustannusdata [:yhteensa :yht-budjetoitu-summa-indeksikorjattu])
