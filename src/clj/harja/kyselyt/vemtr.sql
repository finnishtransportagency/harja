...
CREATE OR REPLACE TEMPORARY VIEW yh_vemtr_toteutuneet AS
  SELECT MIN(tpk4.nimi) as nimi, 0 as jarjestys, 0 as "suunniteltu", 0::smallint as "hoitokauden-alkuvuosi", tpk4.suunnitteluyksikko, tpk4.yksikko, tpk4.id as toimenpidekoodi, 0 as urakka, '' as toimenpide, ROUND(SUM(tt.maara), 2) as toteuma, ROUND(SUM(tm.maara), 2) as "toteutunut-materiaalimaara"
          FROM toteuma t
                   JOIN toteuma_tehtava tt ON t.id = tt.toteuma AND tt.poistettu = FALSE
                   LEFT JOIN toteuma_materiaali tm
                             ON t.id = tm.toteuma AND tm.poistettu = FALSE,
...
select nimi, jarjestys, suunniteltu, "hoitokauden-alkuvuosi", suunnitteluyksikko, yksikko, toimenpidekoodi, toimenpide, toteuma, "toteutunut-materiaalimaara" from mhu_vemtr union select nimi, jarjestys, suunniteltu, "hoitokauden-alkuvuosi", suunnitteluyksikko, yksikko, toimenpidekoodi, toimenpide, toteuma, "toteutunut-materiaalimaara" from yh_vemtr_suunnitellut union select nimi, jarjestys, suunniteltu, "hoitokauden-alkuvuosi", suunnitteluyksikko, yksikko, toimenpidekoodi, toimenpide, toteuma, "toteutunut-materiaalimaara" from yh_vemtr_toteutuneet;
