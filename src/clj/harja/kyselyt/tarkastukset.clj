...
(declare luo-tarkastus<! luodun-tarkastuksen-id paivita-tarkastus!
  ...
  hae-urakan-ei-kayty-tieturvallisuusverkko)
...
(defn- luo-tarkastus*
  [db user urakka-id {:keys [lahde aika tr tyyppi tarkastaja sijainti
...
(defn- paivita-tarkastus*
  [db user urakka-id {:keys [id aika tr tyyppi tarkastaja sijainti
                             ...
                             nayta-urakoitsijalle pisteet alkuperainen_sijainti]} urakoitsija?]
  (log/debug (format "Päivitetään tarkastus id: %s " id))
...
(defn luo-tai-paivita-tarkastus
  ...
  [db user urakka-id {:keys [id yllapitokohde] :as tarkastus}]
