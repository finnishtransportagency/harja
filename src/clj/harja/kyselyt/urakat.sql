...
SELECT u.id, u.nimi, u.tyyppi, u.urakkanro, COALESCE(st_distance(u.alue, st_makepoint(:x, :y)),
...
SELECT u.id,
       ...
       urk.nimi                                                                            AS urakoitsija_nimi,
       urk.ytunnus                                                                         AS urakoitsija_ytunnus,
       (SELECT array_agg(concat(id, '=', sampoid)) FROM sopimus s WHERE urakka = u.id
...
SELECT u.id, u.nimi, u.hallintayksikko, u.sampoid
FROM urakka u
       LEFT JOIN organisaatio urk ON u.urakoitsija = urk.id
WHERE (u.nimi ILIKE :teksti
...
SELECT EXISTS(SELECT id FROM urakka WHERE id = :id);
...
UPDATE urakka
SET tyyppi = :urakkatyyppi :: urakkatyyppi
WHERE hanke = (SELECT id FROM hanke WHERE sampoid = :hanke_sampoid);
...
SET urakoitsija = (SELECT id
                   FROM organisaatio
                   WHERE sampoid = (SELECT urakoitsija_sampoid FROM sopimus WHERE sampoid = :sopimus_sampoid))
WHERE sampoid = (SELECT urakka_sampoid FROM sopimus WHERE sampoid = :sopimus_sampoid
...
WHERE sampoid IN (SELECT urakka_sampoid FROM sopimus WHERE urakoitsija_sampoid = :urakoitsija_sampoid
                                                       AND paasopimus IS NULL);
...
SELECT u.id, u.hanke, u.nimi, lpad(cast(u.urakkanro as varchar), 4, '0') AS alueurakkanro
FROM urakka u
...
SELECT u.alue AS urakka_alue, alueurakka.alue AS alueurakka_alue
FROM urakka u
       LEFT JOIN alueurakka ON u.urakkanro = alueurakka.alueurakkanro
WHERE u.id = :id;

-- name: hae-urakoiden-geometriat
SELECT hoidon_paaurakan_geometria(u.id) AS urakka_alue,
       u.id                             AS urakka_id,
       CASE
...
SELECT id, nimi, alkupvm, loppupvm, tyyppi
FROM urakka
WHERE sampoid = :sampoid;
...
SELECT u.id, st_distance(au.alue, st_makepoint(:x, :y)) AS etaisyys
FROM urakka u
...
-- single?: true
SELECT exists(SELECT id FROM urakka WHERE urakkanro = :urakkanro);
...
-- name: hae-urakan-ely
SELECT o.id, o.nimi, o.elynumero, o.lyhenne
FROM organisaatio o
