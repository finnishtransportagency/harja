...
SELECT
  ...
  MIN(t.lahetysaika)
    FILTER (WHERE t.lahetysaika BETWEEN :alku AND :loppu) AS "ensimmainen-havainto",
  MAX(t.lahetysaika)
    FILTER (WHERE t.lahetysaika BETWEEN :alku AND :loppu) AS "viimeisin-havainto"
FROM
  tyokonehavainto t
...
SELECT
    YRITA_TIEREKISTERIOSOITE_PISTEELLE2( :piste :: GEOMETRY,
  :etaisyys :: INTEGER) AS tr_osoite;
...
FROM
  (SELECT
       ST_MAKELINE( :rp1 :: GEOMETRY,
     :rp2 :: GEOMETRY) AS viiva) v;
...
SELECT
  ...
  t.tehtavat,
  t.tyokonetyyppi,
  ST_MakeLine(array_agg(t.sijainti
              ORDER BY t.lahetysaika ASC) :: GEOMETRY []) AS reitti
FROM tyokonehavainto t
WHERE ST_Contains(ST_MAKEENVELOPE( :xmin, :ymin, :xmax, :ymax), sijainti::GEOMETRY) AND
(t.urakkaid IN (:urakat) OR
-- Jos urakkatietoa ei ole, näytetään vain oman organisaation (tai tilaajalle kaikki)
