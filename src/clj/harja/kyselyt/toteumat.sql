...
SELECT tt.id, -- Jotta "sarakkeet vektoriin" toimii oikein
       tt.id              AS tehtava_id,
       tt.toteuma         AS toteuma_id,
...
VALUES (:toteuma, :toimenpidekoodi, :maara, NOW(), :kayttaja, FALSE, :paivanhinta,
        (CASE
             WHEN :paivanhinta :: NUMERIC IS NULL
                 THEN TRUE
             ELSE FALSE
            END), :urakka_id);
...
SELECT EXISTS(SELECT ulkoinen_id
              FROM toteuma
...
WHERE t.urakka = :urakka
  ...
  AND t.poistettu = false
UNION
-- Haetaan yrittäisinä riveinä käsin lisätyt toteumat
SELECT t.id,
       t.alkanut,
       tt.maara,
...
WHERE tyyppi = :tyyppi :: maksueratyyppi
  AND toimenpideinstanssi IN (SELECT tpi.id
                              FROM toimenpideinstanssi tpi
                                       JOIN toimenpide emo ON emo.id = tpi.toimenpide
                                       JOIN tehtava tpk ON tpk.emo = emo.id
                              WHERE tpk.id = :toimenpidekoodi
                                AND tpi.urakka = :urakka
                                AND tpi.loppupvm > current_timestamp - INTERVAL '3 months');

-- name: merkitse-toteumatehtavien-maksuerat-likaisiksi!
-- Merkitsee toteumaa vastaavan maksuerän likaiseksi: lähtetetään seuraavassa päivittäisessä lähetyksessä
UPDATE maksuera
SET likainen = TRUE
WHERE numero IN (SELECT m.numero
                 FROM maksuera m
                          JOIN toimenpideinstanssi tpi ON tpi.id = m.toimenpideinstanssi AND
                                                          tpi.loppupvm > current_timestamp - INTERVAL '3 months'
                          JOIN toimenpide emo ON emo.id = tpi.toimenpide
                          JOIN tehtava tpk ON tpk.emo = emo.id
                          JOIN toteuma_tehtava tt ON tt.toimenpidekoodi = tpk.id
                          JOIN toteuma t ON t.id = tt.toteuma
                 WHERE tt.id IN (:toteuma_tehtava_idt)
                   AND t.tyyppi :: TEXT = m.tyyppi :: TEXT);
...
SET likainen = TRUE,
    muokattu = current_timestamp
WHERE tyyppi = 'muu'
  AND toimenpideinstanssi IN (SELECT id
                              FROM toimenpideinstanssi
...
SELECT EXISTS(SELECT *
              ...
              WHERE nimi = ANY (ARRAY_REMOVE(ARRAY [:materiaalit]::TEXT[], null))
                AND materiaalityyppi IN ('talvisuola', 'formiaatti'))
           OR
       EXISTS(SELECT * FROM tehtava WHERE id = ANY (ARRAY_REMOVE(ARRAY [:tehtavat]::INT[], null)) AND nimi = 'Suolaus');
...
SELECT hoitoluokka_pisteelle(ST_MakePoint(:x, :y) :: GEOMETRY,
                             ...
                             250::INTEGER,
                             array_remove(ARRAY [:kielletyt_hoitoluokat]::INT[], null)) AS talvihoitoluokka,
...
VALUES (:toteuma, NOW(), :toimenpidekoodi, :maara, :luoja, :paivan_hinta, :lisatieto,
        (CASE
             WHEN :paivan_hinta :: NUMERIC IS NULL
                 THEN TRUE
             ELSE FALSE
            END), :urakka_id);
...
SELECT ST_Simplify(t.reitti, :toleranssi) AS reitti,
       tt.toimenpidekoodi                 AS tehtava_toimenpidekoodi,
       tpk.nimi                           AS tehtava_toimenpide
FROM toteuma_tehtava tt
         JOIN toteuma t ON tt.toteuma = t.id
         JOIN tehtava tpk ON tt.toimenpidekoodi = tpk.id
WHERE t.urakka = :urakka-id
...
SELECT mk.nimi            AS materiaali_nimi,
...
SELECT ST_LineLocatePoint(v.viiva ::geometry,
                          ST_ClosestPoint(v.viiva ::geometry, :piste ::geometry) ::geometry) AS paikka
FROM (SELECT ST_MakeLine(:rp1 ::geometry, :rp2 ::geometry) AS viiva) v;
...
INSERT INTO toteuma_liite (toteuma, liite)
VALUES (:toteuma, :liite);
...
WHERE ((t.toteuma_muutostiedot_muokattu IS NOT NULL AND
    ...
    OR (t.toteuma_muutostiedot_muokattu IS NULL AND
        t.toteuma_muutostiedot_luotu BETWEEN :alkuaika::TIMESTAMP AND :loppuaika::TIMESTAMP))
