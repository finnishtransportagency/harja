...
FROM toteuma t
         ...
         LEFT JOIN materiaalikoodi mk ON tm.materiaalikoodi = mk.id
         LEFT JOIN urakka u on t.urakka = u.id
WHERE (t.alkanut BETWEEN :nyt::TIMESTAMP AND :nyt::TIMESTAMP - '1 day'::interval)
GROUP BY t.id, t.alkanut, u.id
ORDER BY t.alkanut ASC
) ON CONFLICT DO NOTHING; -- Jos vahingossa siirretään sama toteuma toiseen kertaan, niin luovutaan yrityksestä
