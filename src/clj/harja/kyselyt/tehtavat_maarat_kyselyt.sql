...
WITH urakan_vuodet AS (
  ...
  SELECT t.id
   ...
   WHERE (tr.voimassaolo_alkuvuosi  IS NULL OR tr.voimassaolo_alkuvuosi  <= (SELECT urakan_alkuvuosi FROM urakan_vuodet))
     ...
     tulevat_vuodet AS (
  SELECT generate_series((:hoitokauden_alkuvuosi::INTEGER + 1),
                         (SELECT viimeinen_hoitokauden_alkuvuosi FROM urakan_vuodet))::INTEGER AS hoitokauden_alkuvuosi
     ),
...
WITH urakan_vuodet AS (
  ...
  SELECT t.id AS tehtava_id
   ...
   WHERE (tr.voimassaolo_alkuvuosi  IS NULL OR tr.voimassaolo_alkuvuosi  <= (SELECT urakan_alkuvuosi FROM urakan_vuodet))
     ...
     menneet_vuodet AS (
  SELECT generate_series((SELECT urakan_alkuvuosi FROM urakan_vuodet),
                         (:hoitokauden_alkuvuosi::INTEGER - 1))::INTEGER AS hoitokauden_alkuvuosi
     ),
