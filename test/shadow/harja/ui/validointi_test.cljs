...
(deftest hoitokaudella
  (testing "Täyttää säännön hoitokaudella"
    (with-redefs [ur/valittu-hoitokausi (atom [pvm-2015 pvm-2017])]
  ...
  (testing "Ei täytä sääntöä hoitokaudella"
    (with-redefs [ur/valittu-hoitokausi (atom [pvm-2015 pvm-2016])]
...
(deftest validi-tr
  (testing "Täyttää säännön validi-tr"
    (is (nil? (validi-rivi? :validi-tr {:tie 20 :aosa 1 :aet 1}
                {:geometria 123} "viesti" [:geometria])))
    (is (nil? (validi-rivi? :validi-tr {:tie 20 :aosa 1 :losa 1}
                {:geometria 123} "viesti" [:geometria]))))
  (testing "Ei täytä sääntöä validi-tr"
    (is (some? (validi-rivi? :validi-tr {:tie 20 :aosa 1 :aet 1}
                 {:geometria 123} "viesti" [:reitti])))
    (is (some? (validi-rivi? :validi-tr {:tie 0 :aosa 1 :aet 1}
                 {:geometria 123} "viesti" [:geometria])))))
...
(deftest ei-tyhja-jos-toinen-avain-ei-joukossa
  (testing "Täyttää säännön ei-tyhja-jos-toinen-avain-ei-joukossa"
    (is (nil? (validi-rivi? :ei-tyhja-jos-toinen-avain-ei-joukossa
                1 {:id 1} :id [3 1])))
    (is (nil? (validi-rivi? :ei-tyhja-jos-toinen-avain-ei-joukossa
                nil {:id 55} :id [1 2 55]))))
  (testing "Ei täytä sääntöä ei-tyhja-jos-toinen-avain-ei-joukossa"
    (is (some? (validi-rivi? :ei-tyhja-jos-toinen-avain-ei-joukossa
                 nil {:id 11} :id [1 2 55])))))
...
(deftest ei-tyhja-jos-toinen-avain-nil
  (testing "Täyttää säännön ei-tyhja-jos-toinen-avain-nil"
    ...
    (is (nil? (validi-rivi? :ei-tyhja-jos-toinen-avain-nil ""
                {:id 1 :nimi "Seppo"} :nimi))))
...
(deftest ei-avoimia-korjaavia-toimenpiteitä
  (testing "Täyttää säännön ei-avoimia-korjaavia-toimenpiteitä"
    (is (nil? (validi-rivi? :ei-avoimia-korjaavia-toimenpiteitä :suljettu
                {:korjaavattoimenpiteet [{:tila :toteutettu}
                                         {:tila :toteutettu}]})))
    (is (nil? (validi-rivi? :ei-avoimia-korjaavia-toimenpiteitä :kasitelty
                {:korjaavattoimenpiteet [{:tila :toteutettu}
                                         {:tila :toteutettu}]})))
    (is (nil? (validi-rivi? :ei-avoimia-korjaavia-toimenpiteitä :kesken
                {:korjaavattoimenpiteet [{:tila :kesken}
                                         {:tila :toteutettu}]}))))
  (testing "Ei täytä sääntöä ei-avoimia-korjaavia-toimenpiteitä"
    (is (some? (validi-rivi? :ei-avoimia-korjaavia-toimenpiteitä :suljettu
                 {:korjaavattoimenpiteet [{:tila :foo}]})))
    (is (some? (validi-rivi? :ei-avoimia-korjaavia-toimenpiteitä :kasitelty
                 {:korjaavattoimenpiteet [{:tila :bar}]})))
    (is (some? (validi-rivi? :ei-avoimia-korjaavia-toimenpiteitä :suljettu
                 {:korjaavattoimenpiteet [{:tila :foo}]})))
    (is (some? (validi-rivi? :ei-avoimia-korjaavia-toimenpiteitä :kasitelty
                 {:korjaavattoimenpiteet [{:tila :foo}
...
(deftest joku-naista
  (testing "Täyttää säännön joku-naista"
    (is (nil? (validi-rivi? :joku-naista nil
                {:id 1 :nimi "Max" :intohimo :clojure}
                :id :nimi :intohimo)))
    (is (nil? (validi-rivi? :joku-naista nil
                {:id 1 :nimi nil :intohimo nil}
                :id :nimi :intohimo)))
    (is (nil? (validi-rivi? :joku-naista nil
                {:id nil :nimi "Max" :intohimo nil}
                :id :nimi :intohimo)))
    (is (nil? (validi-rivi? :joku-naista nil
                {:id nil :nimi nil :intohimo :clojure}
                :id :nimi :intohimo))))
  (testing "Ei täytä sääntöä joku-naista"
    (is (some? (validi-rivi? :joku-naista nil
                 {:id nil :nimi nil :intohimo nil}
                 :id :nimi :intohimo)))))

(deftest uniikki
  (testing "Täyttää säännön uniikki"
    (is (nil? (validi? :uniikki :intohimo nil nil
                {1 {:id 1 :intohimo :clojure}
                 2 {:id 2 :intohimo :clojure}
                 3 {:id 3 :intohimo :jalkapallo}})))
    (is (nil? (validi? :uniikki :intohimo :clojure nil
                {1 {:id 1 :intohimo :clojure}
                 2 {:id 2 :intohimo :coca-cola}
                 3 {:id 3 :intohimo :jalkapallo}}))))
  (testing "Ei täytä sääntöä uniikki"
    (is (some? (validi? :uniikki :intohimo :clojure nil
                 {1 {:id 1 :intohimo :clojure}
                  2 {:id 2 :intohimo :clojure}
                  3 {:id 3 :intohimo :jalkapallo}})))))
...
(deftest pvm-kentan-jalkeen
  (testing "Täyttää säännön pvm-kentan-jalkeen"
    (is (nil? (validi-rivi? :pvm-kentan-jalkeen pvm-2016
                {:pvm pvm-2015} :pvm))))
  (testing "Ei täytä sääntöä pvm-kentan-jalkeen"
    (is (some? (validi-rivi? :pvm-kentan-jalkeen pvm-2016
                 {:pvm pvm-2017} :pvm)))))
...
(deftest toinen-arvo-annettu-ensin
  ...
  (testing "Ei täytä sääntöä toinen-arvo-annettu-ensin"
    (is (some? (validi-rivi? :toinen-arvo-annettu-ensin 1 {:id 1} :tyyppi)))))

(deftest ei-tyhja-jos-toinen-arvo-annettu
...
(deftest ainakin-toinen-annettu
  (testing "Täyttää säännön ainakin-toinen-annettu"
    (is (nil? (validi-rivi? :ainakin-toinen-annettu nil
                {:id 1 :tyyppi :foo} [:id])))
    (is (nil? (validi-rivi? :ainakin-toinen-annettu nil
                {:id 1 :tyyppi :foo} [:tyyppi])))
    (is (nil? (validi-rivi? :ainakin-toinen-annettu nil
                {:id 1 :tyyppi :foo} [:id :tyyppi]))))
  (testing "Ei täytä sääntöä ainakin-toinen-annettu"
    (is (some? (validi-rivi? :ainakin-toinen-annettu nil
                 {:id 1 :tyyppi :foo} [:nimi :intohimo])))))
