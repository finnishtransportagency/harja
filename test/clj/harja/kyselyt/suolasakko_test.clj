...
(defn suolasakko [ur lampotila lampotila-pitka sakko-per-tonni sallittu-maara kaytetty-maara]
  ...
  (u (str "INSERT INTO toteuma_materiaali (toteuma, luotu, materiaalikoodi, maara, hoitokauden_alkuvuosi)
  VALUES ((SELECT id FROM toteuma WHERE lisatieto = 'Suolasakkototeuma'), NOW(),
  (SELECT id FROM materiaalikoodi WHERE nimi='Talvisuola, rakeinen NaCl')," kaytetty-maara ", 2014)"))
