...
(deftest siirra-analytiikan-toteumat-toimii-vuodelle-2015
  (let [testitietokanta (:db jarjestelma)
        ...
        siirretty (first (q "SELECT luotu, toteumamateriaalit FROM analytiikka_toteumat WHERE toteuma_tunniste_id = '1117'"))
        siirron-luotu-aikaleima (first siirretty)
        toteumamateriaalit (first (konversio/jsonb->clojuremap (second siirretty)))]
