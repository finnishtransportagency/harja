...
(defn- tarkista-muutos-kulu-on-validi
  "Tarkistaa että vastaus on validi, ja kulu tallennettiin"
...
(deftest muutos-kulun-tallennus-sekä-validointi-toimii
  (let [urakka (hae-urakan-id-nimella "Iin MHU 2021-2026")
        tpi (hae-toimenpideinstanssi-id urakka "14301")
        hoitokaudet (mapv (fn [vuosi]
                            [(pvm/hoitokauden-alkupvm vuosi)
                             (pvm/paivan-lopussa (pvm/hoitokauden-loppupvm (inc vuosi)))]) (range 2021 2026))

        tallenna-kulu-fn (fn [params]
                           (kutsu-palvelua (:http-palvelin jarjestelma)
                             :tallenna-kulu +kayttaja-jvh+ params))

        hae-muutostyo-fn (fn []
                           (let [vastaus (kutsu-palvelua
                                           (:http-palvelin jarjestelma)
                                           :hae-urakan-muutostyot +kayttaja-jvh+
                                           {:urakka-id urakka
                                            :valittu-hoitokausi (last hoitokaudet)})
                                 ;; Palauta ensimmäinen erillisrahoitettu muutostyö 
                                 ;; mikä kannasta löytyy
        ...
        erillisrahoitettu-muutostyo (hae-muutostyo-fn)
        ...
        paivita-erapaivat-fn (fn [muutostyo-voimassa kulu-erapaiva]
                               (let [;; Päivitä muutoksen voimassa alkaen tietokantaan
                                     _ (i (format "UPDATE mhu_muutos SET voimassa_alkaen = '%s' WHERE id = %s;"
                                            muutostyo-voimassa
                                            (:id erillisrahoitettu-muutostyo)))
                                     ;; Hae päivitetyt muutostyöt
                                     paivitetty-muutostyo (hae-muutostyo-fn)]
    ...
    (testing "Muutostyön kulun kirjaus toimii, sama voimassa alkaen ja eräpäivä -> OK"
      (let [muutostyo-voimassa "2025-10-02"        ;; <- sama kun eräpäivä
            ...
            vastaus (tallenna-kulu-fn {:urakka-id     urakka
    ...
    (testing "Muutostyön kulun kirjaus heittää virheen, eräpäivä on voimassa alkaen ennen -> VIRHE"
      (let [muutostyo-voimassa "2025-10-03"
            ...
            vastaus (try
                      (tallenna-kulu-fn {:urakka-id     urakka
                                         :kulu-kohdistuksineen uusi-muutos-kulu})
                      (catch Exception e
                        e))]
        (is (true? (str/includes? vastaus odotettu-poikkeus))
