...
(defn- erilliskustannukset-sql-haku [urakka alkupvm loppupvm]
...
(defn- hoidonjohdonpalkkio-sql-haku [urakka alkupvm loppupvm]
  (str "WITH urakan_toimenpideinstanssi_23150 AS
         (SELECT tpi.id AS id
          FROM toimenpideinstanssi tpi
                   JOIN toimenpidekoodi tpk3 ON tpk3.id = tpi.toimenpide
...
(defn- johto-ja-hallintokorvaukset-sql-haku [urakka alkupvm loppupvm]
  (str "WITH urakan_toimenpideinstanssi_23150 AS
         (SELECT tpi.id AS id
          FROM toimenpideinstanssi tpi
                   JOIN toimenpidekoodi tpk3 ON tpk3.id = tpi.toimenpide
...
(defn- hankintakustannukset-sql-haku [urakka alkupvm loppupvm]
  (str "
  SELECT
     kt.summa,
...
(deftest budjetoidut-erillishankinnat-test
  (let [urakka-id (hae-oulun-maanteiden-hoitourakan-2019-2024-id)
        ...
        erillishankinnat-sql (q (erilliskustannukset-sql-haku urakka-id alkupvm loppupvm))
...
(deftest budjetoidut-hoidonjohdonpalkkiot-test
  (let [urakka-id (hae-oulun-maanteiden-hoitourakan-2019-2024-id)
        ...
        hj-sql (q (hoidonjohdonpalkkio-sql-haku urakka-id alkupvm loppupvm))
...
(deftest budjetoidut-johtoja-hallintokorvaukset-test
  (let [urakka-id (hae-oulun-maanteiden-hoitourakan-2019-2024-id)
        ...
        jjh-sql (q (johto-ja-hallintokorvaukset-sql-haku urakka-id alkupvm loppupvm))
...
(deftest budjetoidut-hankintakustannukset-test
  (let [urakka-id (hae-oulun-maanteiden-hoitourakan-2019-2024-id)
        ...
        h-sql (q (hankintakustannukset-sql-haku urakka-id alkupvm loppupvm))
