...
(deftest tallenna-kokonaishintainen-toteuma-testi
  (let [tyon-pvm (konv/sql-timestamp (pvm/luo-pvm 2020 11 24)) ;;24.12.2020
    ...
    (is (= (get-in lisatty [:yksikko]) "jkm") "Yksikkö")
...
(deftest hae-urakan-kokonaishintaisten-toteumien-tehtavien-paivakohtaiset-summat
  (let [urakka-id (hae-oulun-alueurakan-2014-2019-id)
        ...
        odotettu [{:pvm #inst "2017-01-31T22:00:00.000-00:00", :toimenpidekoodi 1369, :maara 666M, :jarjestelmanlisaama true, :nimi "Liukkaudentorjunta suolaamalla (materiaali)", :yksikko "jkm"}
                  {:pvm #inst "2015-01-31T22:00:00.000-00:00", :toimenpidekoodi 1369, :maara 123M, :jarjestelmanlisaama true, :nimi "Liukkaudentorjunta suolaamalla (materiaali)", :yksikko "jkm"}]
...
(deftest toteumien-insertit-partitioille
  (let [urakka-id (hae-urakan-id-nimella "Iin MHU 2021-2026")]
    ...
    (let [maara-partitio-2025-h1 (ffirst (q (format "SELECT count(*) FROM toteuma_250101_250701 WHERE lisatieto = 'part-test' AND urakka = %s;" urakka-id)))
          ...
          maara-partitio-2026-h2 (ffirst (q (format "SELECT count(*) FROM toteuma_260701_270101 WHERE lisatieto = 'part-test' AND urakka = %s;" urakka-id)))]
      (is (= maara-partitio-2025-h1 3))
      (is (= maara-partitio-2025-h2 3))
      (is (= maara-partitio-2026-h1 4))
      (is (= maara-partitio-2026-h2 4))

      ;; siirretään kaikki vuodelle 2028 H1, niin 2025 ja 2026 pitää nollautua ja kaikki siirtyä uuteen partitioon
      (u (format "UPDATE toteuma set alkanut = '2028-2-2' WHERE lisatieto = 'part-test' AND urakka = %s;" urakka-id))
      (let [maara-partitio-2025-h1-siirretty (ffirst (q (format "SELECT count(*) FROM toteuma_250101_250701 WHERE lisatieto = 'part-test' AND urakka = %s;" urakka-id)))
            maara-partitio-2025-h2-siirretty (ffirst (q (format "SELECT count(*) FROM toteuma_250701_260101 WHERE lisatieto = 'part-test' AND urakka = %s;" urakka-id)))
            maara-partitio-2026-h1-siirretty (ffirst (q (format "SELECT count(*) FROM toteuma_260101_260701 WHERE lisatieto = 'part-test' AND urakka = %s;" urakka-id)))
            maara-partitio-2026-h2-siirretty (ffirst (q (format "SELECT count(*) FROM toteuma_260701_270101 WHERE lisatieto = 'part-test' AND urakka = %s;" urakka-id)))]
        (is (= maara-partitio-2025-h1-siirretty 0))
        (is (= maara-partitio-2025-h2-siirretty 0))
        (is (= maara-partitio-2026-h1-siirretty 0))
        (is (= maara-partitio-2026-h2-siirretty 0))
        (is (= 14 (ffirst (q (format "SELECT count(*) FROM toteuma_280101_280701 WHERE lisatieto = 'part-test' AND urakka = %s;" urakka-id)))))))))
