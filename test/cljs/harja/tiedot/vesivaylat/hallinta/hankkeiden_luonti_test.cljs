...
(deftest tallentamisen-aloitus
  (vaadi-async-kutsut
    ...
    (is (true? (:tallennus-kaynnissa? (e! (h/->TallennaHanke {:id 1}) {:haetut-hankkeet []}))))))

(deftest tallentamisen-valmistuminen
  (testing "Uuden hankkeen tallentaminen"
    (let [vanhat [{:id 1} {:id 2}]
          uusi {:id 3}
          tulos (e! (h/->HankeTallennettu uusi) {:haetut-hankkeet vanhat})]
...
(deftest hakemisen-valmistuminen
  (let [tulos (e! (h/->HankkeetHaettu [{:id 1}]) {:haetut-hankkeet []})]
