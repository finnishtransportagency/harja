...
(deftest tallentamisen-aloitus
  (vaadi-async-kutsut
    ...
    (is (true? (:tallennus-kaynnissa? (e-tila! s/->TallennaSopimus {:id 1} {:haetut-sopimukset []}))))))

(deftest tallentamisen-valmistuminen
  (testing "Uuden sopimuksen tallentaminen"
    (let [vanhat [{:id 1} {:id 2}]
          uusi {:id 3}
          tulos (e-tila! s/->SopimusTallennettu uusi {:haetut-sopimukset vanhat})]
...
(deftest hakemisen-valmistuminen
  (let [tulos (e-tila! s/->SopimuksetHaettu [{:id 1}] {:haetut-sopimukset []})]
