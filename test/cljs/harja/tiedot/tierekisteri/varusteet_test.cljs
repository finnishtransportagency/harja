...
(deftest kytke-haku-paalle
  (binding [tuck/*current-send-function* :D]
    (let [alku-app {:hakuehdot {:haku-kaynnissa? false
                                :tunniste "testi0001"
                                :tietolaji "tl506"}}
          {{haku-kaynnissa? :haku-kaynnissa?} :hakuehdot} (tuck/process-event (tiedot/->HaeVarusteita) alku-app)]
...
(deftest tarkista-varustehaun-tulosten-kasittely
  (let [alku-app {:hakuehdot {:haku-kaynnissa? false
        ...
        {varusteet :varusteet {haku-kaynnissa? :haku-kaynnissa?} :hakuehdot} (tuck/process-event (tiedot/->VarusteHakuTulos tietolaji hakutulokset) alku-app)]
    (is (not haku-kaynnissa?) "Haku on kytketty pois päältä")
    (is (= (count hakutulokset) (count varusteet)) "Varusteita merkittiin oikea määrä")))
