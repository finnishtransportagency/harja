...
cy.route('POST', '_/tehtavamaarat-hierarkiassa').as('tehtavamaarat')
  ...
  it('Hoitokautta voi vaihtaa', () => {
    cy.get('table.grid').contains('Sorateiden pölynsidonta').parent().find('td.muokattava').find('input').clear().type('667').blur()
    cy.get('div.select-default').contains('1.10.2021-30.9.2022').click()
    cy.contains('1.10.2023-30.9.2024').click()
    cy.wait(2000)
    cy.get('table.grid').contains('667').should('not.exist')
    cy.get('div.select-default').contains('1.10.2023-30.9.2024').click()
    cy.get('.harja-alasvetolistaitemi').contains('1.10.2021-30.9.2022').click()
    cy.wait(2000)
