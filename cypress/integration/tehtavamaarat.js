...
cy.route('POST', '_/tehtavamaarat-hierarkiassa').as('tehtavamaarat')
  ...
  it('Hoitokautta voi vaihtaa', () => {
    cy.server()
    cy.route('POST', '_/tehtavamaarat-hierarkiassa').as('tehtavamaarat')
    cy.get('table.grid').contains('Sorateiden pölynsidonta').parent().find('td.muokattava').find('input').type('667')
    cy.get('div.select-default').contains('01.10.2021-30.9.2022').click()
    cy.contains('1.10.2023-30.9.2024').click()
    cy.wait('@tehtavamaarat')
    cy.wait(2000)
    cy.get('table.grid').contains('667').should('not.exist')
    cy.get('div.select-default').contains('01.10.2023-30.9.2024').click()
    cy.get('.harja-alasvetolistaitemi').contains('01.10.2022-30.9.2023').click()
    cy.wait('@tehtavamaarat')
    cy.wait(2000)
