...
cy.route('POST', '_/tehtavamaarat-hierarkiassa').as('tehtavamaarat')
  ...
  it('Määrän voi syöttää', () => {
    cy.get('table.grid').contains('Levähdys- ja pysäköimisalueet').parent().find('td.muokattava').find('input').type('666')
  })
  ...
  it('Toimenpidettä voi vaihtaa', () => {
    ...
    cy.wait(2000)
    cy.get('table.grid').contains('Levähdys- ja pysäköimisalueet').should('not.exist')
  })
  ...
  after(() => {
    ...
    cy.wait(1000)
    cy.get('table.grid').contains('Levähdys- ja pysäköimisalueet').parent().find('td.muokattava').find('input').clear()
